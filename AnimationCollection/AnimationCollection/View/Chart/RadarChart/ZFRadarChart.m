...
/** 存储itemLabel颜色的数组 */
@property (nonatomic, strong) NSMutableArray * itemColorArray;
/** 存储itemLabel的数组 */
...
- (void)setItemLabelOnChart{
    for (NSInteger i = 0; i < self.radar.itemLabelCenterArray.count; i++) {
        ...
        CGSize size = [self.radar.itemArray[i] stringWidthRectWithSize:CGSizeMake(self.bounds.size.width - self.radar.radius - 10, self.bounds.size.width / 2 - self.radar.radius - 10) font:_itemFont].size;
        ...
        itemLabel.textColor = _itemColorArray[i];
...
- (void)strokePath{
    ...
    if ([self.dataSource respondsToSelector:@selector(itemColorArrayInRadarChart:)]) {
        _itemColorArray = [NSMutableArray arrayWithArray:[self.dataSource itemColorArrayInRadarChart:self]];
        self.radar.separateColorArray = _itemColorArray;

    }else{
        _colorArray = [NSMutableArray arrayWithArray:[[ZFMethod shareInstance] cachedRandomColor:_valueArray]];
    }
    
//    if ([self.dataSource respondsToSelector:@selector(separateLineColorArrayInRadarChart:)]) {
//        _itemColorArray = [NSMutableArray arrayWithArray:[self.dataSource itemColorArrayInRadarChart:self]];
//    }else{
//        _colorArray = [NSMutableArray arrayWithArray:[[ZFMethod shareInstance] cachedRandomColor:_valueArray]];
//    }
